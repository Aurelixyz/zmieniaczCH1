...
# ==========================================
# ODTWARZANIE MP3 / WAV BEZ ZEWN. BIBLIOTEK
# ==========================================
def play_custom_audio(file_path):
    try:
        ...
        ctypes.windll.winmm.mciSendStringW('play alertaudio', None, 0, None)
...
APP_CONFIG = {
    ...
    # KONFIGURACJA ALERTÓW (Indywidualna dla 3 obszarów)
    ...
    # POZYCJA I WYMIARY OKNA Z PRZYWRACANIA
    ...
    "hotkeys": {
        ...
        "show_gui_key": None,  # NOWOŚĆ: Skrót przywracający okno
        "next_ch": 0x09,
        "prev_ch": None,
...
VK_DICT = {
    ...
    0x13: "PAUSE", 0x2C: "PRTSC", 0x91: "SCRLK",  # Zmapowane brakujące klawisze
...
def horse_attack_worker():
    ...
    SPACE_SCANCODE = 0x39
    while True:
        if is_horse_attacking and APP_CONFIG.get("is_running", True) and not is_changing_ch:
        ...
        if APP_CONFIG.get("is_running", True):
            with mss.mss() as sct:
                for i in range(1, 4):
                    ...
                    if time.time() < cooldowns[i]: continue
                    ...
                    if monitor and monitor.get("width", 0) > 0 and template is not None:
                        try:
                            ...
                            # Logika wystąpienia
                            if cond == "appears" and max_val > 0.8:
                                is_triggered = True
                            elif cond == "disappears" and max_val < 0.6:
                                is_triggered = True
                            ...
                            if is_triggered:
                                ...
                                if sound_mode != "Brak":
                                    if sound_mode == "Windows: Exclamation":
                                        ...
                                        if os.path.exists(path): play_custom_audio(path)
                                ...
                                cooldowns[i] = time.time() + 5.0  # Cooldown na 5 sek po trafieniu
...
class ChangerHUD(QWidget):
    ...
    def setup_automation_tab(self):
        ...
        # Reszta automatyzacji jak dawniej, skrócona w celu czytelności kodu w UI (zmieści się)
        ...
        self.chk_hold_key = QCheckBox("Trzymaj wciśnięty klawisz: ")
        ...
        for i in range(1, 4):
            ...
            g_layout.addWidget(QLabel("Akcja (Skrypt):"), 2, 0)
            cb_act = QComboBox()
    ...
    def refresh_automation_ui(self):
        buttons = [
            ("auto_post_key", getattr(self, "btn_post_key", None)),
    ...
    def setup_settings_tab(self):
        ...
        l_win.addWidget(QLabel("Szerokość:"), 3, 0)
        self.spin_w = QSpinBox()
        self.spin_w.setRange(200, 2000)
        self.spin_w.setValue(APP_CONFIG.get("gui_rect", {}).get("w", 650))
        self.spin_w.valueChanged.connect(lambda v: [APP_CONFIG["gui_rect"].update({"w": v}), save_config_to_file()])
        l_win.addWidget(self.spin_w, 3, 1)

        l_win.addWidget(QLabel("Wysokość:"), 4, 0)
        self.spin_h = QSpinBox()
        self.spin_h.setRange(200, 2000)
        self.spin_h.setValue(APP_CONFIG.get("gui_rect", {}).get("h", 850))
        self.spin_h.valueChanged.connect(lambda v: [APP_CONFIG["gui_rect"].update({"h": v}), save_config_to_file()])
        l_win.addWidget(self.spin_h, 4, 1)
