...
# ROZSZERZONY ZBIÓR KLAWISZY (Tłumaczenie Hex na Tekst)
VK_DICT = {
...
# ROZSZERZONY ZBIÓR TŁUMACZEŃ DO PYDIRECTINPUT
VK_TO_PDI = {
...
def load_templates():
    global state_templates, btn_templates, alert_templates
...
# ==========================================
# SYSTEM BEZPIECZNEGO PAUZOWANIA I WZNAWIANIA
# ==========================================
def bot_pause():
    APP_CONFIG["is_running"] = False

    # Puszczamy klawisz z farming'u, aby odblokować komputer
...
def bot_resume():
    ...
    # Wracamy do wciśniętego klawisza farming'u, jeżeli ma to odpalone
    vk = APP_CONFIG["hotkeys"].get("auto_hold_key")
    if vk and APP_CONFIG.get("hold_key_enabled"):
        pdi_down(vk)
...
def press_combo_ctypes(vk1, vk2):
    sc1 = ctypes.windll.user32.MapVirtualKeyW(vk1, 0)
    sc2 = ctypes.windll.user32.MapVirtualKeyW(vk2, 0)
    if sc1 and sc2:
        press_key_hardware(sc1)
        time.sleep(0.05)
        press_key_hardware(sc2)
        time.sleep(0.05)
        release_key_hardware(sc2)
        time.sleep(0.05)
        release_key_hardware(sc1)
        time.sleep(0.1)
...
def press_single_ctypes(vk):
    ...
    if sc:
        press_key_hardware(sc)
        time.sleep(0.05)
        release_key_hardware(sc)
        time.sleep(0.1)
                ...
                for i in range(1, 4):
                    ...
                    if monitor and monitor.get("width", 0) > 0 and template is not None:
                        try:
                            ...
                            if max_val > 0.8:
                                sound_mode = APP_CONFIG.get("alert_sounds", {}).get(str(i), "Brak")
                                if sound_mode != "Brak":
                                    ...
                                    elif sound_mode == "Własny plik (.wav)...":
                                        path = APP_CONFIG.get("alert_custom_paths", {}).get(str(i), "")
                                        if os.path.exists(path):
                                            # Flaga SND_ASYNC gwarantuje, że program nie zamarznie na czas odtwarzania
                                            winsound.PlaySound(path, winsound.SND_FILENAME | winsound.SND_ASYNC)

                                if APP_CONFIG.get("alert_pause_bot", False):
                                    bot_pause()
...
def execute_auto_skills():
    # 1. Zatrzymanie działań całego programu
    bot_pause()
    time.sleep(0.2)

    # 2. Strzał sekwencji
    try:
        press_combo_ctypes(0x11, 0x47)  # Ctrl + G
        press_single_ctypes(0x70)  # F1
        press_combo_ctypes(0x11, 0x47)  # Ctrl + G
        time.sleep(0.2)
        press_combo_ctypes(0x11, 0x47)  # Ctrl + G
        press_single_ctypes(0x71)  # F2
        press_combo_ctypes(0x11, 0x47)  # Ctrl + G
    except Exception as e:
        print(f"Błąd Auto Skille: {e}")
    finally:
        # 3. Wznowienie wszystkiego po zakończeniu
        bot_resume()
...
def handle_editor_capture(mode_index):
    ...
    if mode_index >= 20:
        ...
        if len(editor_points) == 0:
            ...
            editor_message = f"🎯 Zapisano Punkt 1 (Lewy-Górny) dla Alertu {alert_num}."
        else:
            editor_points.append((x, y))
            ...
            if not is_img:
                ...
                save_config_to_file()
                editor_message = f"✅ Zapisano obszar dla Alert {alert_num}!"
            else:
                with mss.mss() as sct:
                    img_bgra = np.array(sct.grab(monitor))
                    img_gray = cv2.cvtColor(img_bgra, cv2.COLOR_BGRA2GRAY)
                ...
                load_templates()
                editor_message = f"✅ Zapisano obrazek dla Alert {alert_num}!"
...
class ChangerHUD(QWidget):
    ...
    def change_alert_sound(self, alert_idx, combo_idx, combo_widget):
        sound_opts = ["Brak", "Windows: Exclamation", "Windows: Asterisk", "Windows: Hand", "Własny plik (.wav)..."]
        selected = sound_opts[combo_idx]

        if selected == "Własny plik (.wav)...":
            path, _ = QFileDialog.getOpenFileName(self, f"Wybierz plik dźwiękowy dla Alertu {alert_idx}", "",
                                                  "Pliki WAV (*.wav)")
            if path:
                if "alert_custom_paths" not in APP_CONFIG: APP_CONFIG["alert_custom_paths"] = {}
                APP_CONFIG["alert_custom_paths"][str(alert_idx)] = path
            else:
                # W przypadku anulowania wyboru, cofamy index
                combo_widget.setCurrentText(APP_CONFIG.get("alert_sounds", {}).get(str(alert_idx), "Brak"))
                return

        if "alert_sounds" not in APP_CONFIG: APP_CONFIG["alert_sounds"] = {}
        APP_CONFIG["alert_sounds"][str(alert_idx)] = selected
        save_config_to_file()
    ...
    def setup_automation_tab(self):
        ...
        h_auto.addWidget(QLabel("co (sek):"))
        ...
        lbl5 = QLabel("Auto Dopalacze (cykliczne używanie)")
        lbl5.setStyleSheet("color: #FF9800; font-size: 15px;")
        layout.addWidget(lbl5)

        h_booster = QHBoxLayout()
        self.chk_auto_booster = QCheckBox("Używaj klawisza")
        ...
        self.spin_booster_interval.setSingleStep(1.0)
        ...
        h_booster.addWidget(QLabel("sekund."))
        h_booster.addStretch()
        layout.addLayout(h_booster)

        layout.addWidget(QFrame(frameShape=QFrame.Shape.HLine))

        # Zaktualizowana sekcja dla dźwięków i alertów
        lbl6 = QLabel("Alerty Ekranowe (Dodaj obszary i obrazy w zakładce Edytor)")
        lbl6.setStyleSheet("color: #F44336; font-size: 15px;")
        layout.addWidget(lbl6)

        grid_alerts = QGridLayout()
        sound_opts = ["Brak", "Windows: Exclamation", "Windows: Asterisk", "Windows: Hand", "Własny plik (.wav)..."]
        self.alert_combos = {}
        for i in range(1, 4):
            grid_alerts.addWidget(QLabel(f"Odtwarzaj Dźwięk Alertu {i}:"), i - 1, 0)
            cb = QComboBox()
            cb.addItems(sound_opts)
            curr = APP_CONFIG.get("alert_sounds", {}).get(str(i), "Brak")
            cb.setCurrentText(curr if curr in sound_opts else "Brak")
            cb.currentIndexChanged.connect(lambda idx, a_idx=i, c_box=cb: self.change_alert_sound(a_idx, idx, c_box))
            grid_alerts.addWidget(cb, i - 1, 1)
            self.alert_combos[i] = cb

        layout.addLayout(grid_alerts)

        self.chk_alert_pause = QCheckBox("Zatrzymaj bota (wszystkie funkcje), jeśli pojawi się KTÓRYKOLWIEK alert")
        self.chk_alert_pause.setChecked(APP_CONFIG.get("alert_pause_bot", False))
        self.chk_alert_pause.toggled.connect(
            lambda checked: [APP_CONFIG.update({"alert_pause_bot": checked}), save_config_to_file()])
        layout.addWidget(self.chk_alert_pause)
    ...
    def refresh_automation_ui(self):
        buttons = [
            ...
            ("auto_booster_key", self.btn_booster_key)
        ]
    ...
    def toggle_post_space(self, checked):
        ...
        if not checked: is_horse_attacking = False
        save_config_to_file()
    ...
    def setup_settings_tab(self):
        ...
        info = QLabel("Konfiguracja Głębokich Klawiszy (Sprzętowych)")
        ...
        actions = [
            ("global_pause_key", "Pauza/Wznowienie (Global)"),
    ...
    def start_binding(self, action_id):
        ...
        # Zapobiega wirtualnemu trzymaniu klawisza przed wejściem w nasłuch sprzętowy
        vk_hold = APP_CONFIG["hotkeys"].get("auto_hold_key")
        if vk_hold: pdi_up(vk_hold)
    ...
    def check_hardware_keys(self):
        ...
        ignored_actions = ["auto_post_key", "auto_hold_key", "indep_action_key", "auto_booster_key"]
        ...
        for action, vk in hotkeys.items():
            ...
            if not is_running and action != "global_pause_key":
                continue
    ...
    def update_ui(self):
        ...
        if APP_CONFIG.get("auto_ch_enabled") and not is_changing_ch and is_running:
    ...
    def setup_editor_tab(self):
        ...
        for i in range(1, 4):
            self.combo_editor.addItems([
                f"{19 + (i - 1) * 2 + 1}. Obszar na ekranie: Alert {i} (Skan)",
                f"{19 + (i - 1) * 2 + 2}. Obrazek wycinka: Alert {i} (Porównanie)"
            ])
...
if __name__ == '__main__':
    ...
    threading.Thread(target=auto_booster_worker, daemon=True).start()
