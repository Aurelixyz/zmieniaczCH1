...
import json
import os
import winsound  # Dodano do obsługi dźwięków
...
APP_CONFIG = {
    ...
    # 3 NOWE OBSZARY ALERTÓW
    "alert_1_monitor": {},
    "alert_2_monitor": {},
    "alert_3_monitor": {},
    "alert_play_sound": False,
    ...
    "hotkeys": {
        "global_pause_key": None,  # NOWE: Globalna pauza
        "macro_seq_key": None,  # NOWE: Sekwencja specjalna
        "auto_booster_key": None,  # NOWE: Auto dopalacze
        "next_ch": 0x09,
        "prev_ch": None,
                ...
                for i in range(1, 4):
                    ...
                    if monitor and monitor.get("width", 0) > 0 and template is not None:
                        try:
                            ...
                            if max_val > 0.8:  # Próg 80% pewności
                                if APP_CONFIG.get("alert_play_sound", False):
                                ...
                                if APP_CONFIG.get("alert_pause_bot", False):
                                    ...
                                    is_horse_attacking = False
                                    is_changing_ch = False
                                time.sleep(3.0)  # Zabezpieczenie anty-spamowe alertu
                        except Exception:
                            pass
        time.sleep(1.0)  # Sprawdza alerty co 1 sekundę by nie obciążać procesora


def execute_macro_sequence():
    global is_horse_attacking, is_changing_ch

    # Natychmiastowe ubicie wszystkich procesów bota
    APP_CONFIG["is_running"] = False
    is_horse_attacking = False
    is_changing_ch = False
    time.sleep(0.2)

    # SEKWENCJA: Ctrl+G -> F1 -> Ctrl+G -> Ctrl+G -> F2 -> Ctrl+G
    try:
...
class ChangerHUD(QWidget):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 620, 850)  # Zwiększono wysokość okna
    ...
    def setup_automation_tab(self):
        ...
        self.chk_alert_sound = QCheckBox("Odtwarzaj dźwięk powiadomienia Windows, jeśli pojawi się obraz")
        self.chk_alert_sound.setChecked(APP_CONFIG.get("alert_play_sound", False))
        self.chk_alert_sound.toggled.connect(
            lambda checked: [APP_CONFIG.update({"alert_play_sound": checked}), save_config_to_file()])
        layout.addWidget(self.chk_alert_sound)

        self.chk_alert_pause = QCheckBox("Zatrzymaj bota (wszystkie funkcje), jeśli pojawi się obraz")
    ...
    def setup_settings_tab(self):
        ...
        # Zaktualizowana lista o makro i globalną pauzę
        actions = [
            ("global_pause_key", "Pauza/Wznowienie (Global)"),
            ("macro_seq_key", "Makro Ratunkowe (Ctrl+G...)"),
    ...
    def check_hardware_keys(self):
        ...
        for action, vk in hotkeys.items():
            ...
            # Twarda weryfikacja - jeśli wciśnięto przycisk pauzy, zawsze ma działać
    ...
    def trigger_hotkey_action(self, action):
        if action == "global_pause_key":
            ...
            threading.Thread(target=execute_macro_sequence, daemon=True).start()
        elif action == "next_ch":
    ...
    def update_ui(self):
        ...
        # Szybka weryfikacja i synchronizacja koloru przycisku zatrzymania dla wątków w tle
        is_running = APP_CONFIG.get("is_running", True)
    ...
    def refresh_editor_preview(self):
        ...
        # Obsługa nowych Alertów (indeksy 20 i w górę)
        if idx >= 20:
            rel_idx = idx - 20
...
if __name__ == '__main__':
    ...
    # Odpalanie wątków roboczych w tle
